...
func createDetachedFsMount(m *configs.Mount, flags int) (_ *mountSource, retErr error) {
	fstype := m.Device
	if fstype == "cgroup" {
		fstype = "cgroup2"
	}
	...
	if err := unix.FsconfigSetString(fsfd, "source", fstype); err != nil {
		return nil, &os.PathError{Op: "fsconfigset", Path: fstype, Err: err}
	}
...
func createDetachedMountSource(m *configs.Mount, flags int) (_ *mountSource, retErr error) {
	if m.IsBind() {
		return createDetachedBindMount(m)
	}
	return createDetachedFsMount(m, flags)
}
